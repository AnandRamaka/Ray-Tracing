...
Camera::Camera(Vector3D point_to_look_from, Vector3D point_to_look_at, double vertical_fov, double aspect_ratio) : origin_(point_to_look_from), include_focus_(false) {
...
Camera::Camera(Vector3D point_to_look_from, Vector3D point_to_look_at, double vertical_fov, double aspect_ratio, double distance_to_focus, double aperture) : origin_(point_to_look_from), include_focus_(true), thin_lens_radius_(aperture / 2.0) {
    // Implement a vertical fov. This determines the plane that the rays intersect with. 
    // The larger the fov, the more of the scene will be present.
...
Ray Camera::GetRayAt(double u, double v) {
    if (include_focus_) {
        ...
        Vector3D rd = thin_lens_radius_ * randomInUnitDisk();
